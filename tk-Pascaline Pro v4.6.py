...
def evaluate_node(node):
    """Recursively evaluates the AST nodes."""
...
def export_history():
    ...
    if not history_data:
        ...
        return
    file_path = filedialog.asksaveasfilename(defaultextension=".txt", filetypes=[("Text files", "*.txt")], title="Export History")
    if file_path:
        try:
            with open(file_path, "w", encoding="utf-8") as f:
                f.write(f"--- Pascaline Pro History ({datetime.now().strftime('%Y-%m-%d')}) ---\n")
                for line in history_data: f.write(line + "\n")
...
# Standard Grid
buttons = [
    ("7", "#e0e0e0", "#bdbdbd"), ("8", "#e0e0e0", "#bdbdbd"), ("9", "#e0e0e0", "#bdbdbd"), ("/", "#e0e0e0", "#bdbdbd"),
    ("4", "#e0e0e0", "#bdbdbd"), ("5", "#e0e0e0", "#bdbdbd"), ("6", "#e0e0e0", "#bdbdbd"), ("*", "#e0e0e0", "#bdbdbd"),
    ("1", "#e0e0e0", "#bdbdbd"), ("2", "#e0e0e0", "#bdbdbd"), ("3", "#e0e0e0", "#bdbdbd"), ("-", "#e0e0e0", "#bdbdbd"),
    ("C", "#f44336", "#ef5350"), ("0", "#e0e0e0", "#bdbdbd"), ("=", "#4CAF50", "#66BB6A"), ("+", "#e0e0e0", "#bdbdbd")
]

r, c = 1, 0
for text, color, hov in buttons:
    if text == "=": cmd = calculate_result
    elif text == "C": cmd = clear_display
    else: cmd = (lambda x=text: on_button_click(x))

    btn = tk.Button(calc_frame, text=text, width=5, height=2, font=("Arial", 12, "bold"), bg=color, command=cmd)
...
for i, (txt, cmd, col, hov) in enumerate(specials):
    btn = tk.Button(calc_frame, text=txt, width=5, height=2, font=("Arial", 12, "bold"), bg=col, fg="white" if i < 3 else "black", command=cmd)
    btn.grid(row=5, column=i, padx=2, pady=2)
    apply_hover(btn, col, hov)

# Parentheses (Row 6)
btn_l = tk.Button(calc_frame, text="(", width=12, height=1, font=("Arial", 11, "bold"), bg="#BDBDBD", command=lambda: on_button_click("("))
btn_l.grid(row=6, column=0, columnspan=2, padx=2, pady=2)
apply_hover(btn_l, "#BDBDBD", "#9E9E9E")

btn_r = tk.Button(calc_frame, text=")", width=12, height=1, font=("Arial", 11, "bold"), bg="#BDBDBD", command=lambda: on_button_click(")"))
...
btn_exp = tk.Button(history_frame, text="💾 Export History", command=export_history, bg="#2196F3", fg="white", font=("Arial", 9, "bold"))
btn_exp.pack(fill="x", pady=2)
apply_hover(btn_exp, "#2196F3", "#1976D2")

btn_clr = tk.Button(history_frame, text="🗑️ Clear History", command=clear_history, bg="#f44336", fg="white", font=("Arial", 9, "bold"))
